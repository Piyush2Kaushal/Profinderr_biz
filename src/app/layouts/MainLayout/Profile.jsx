...
export function Profile() {
  ...
  const profileName = user?.companyName || "User";
  const profileSubtitle = user?.accountType || "Account";
  const logo = user?.logo || "/images/200x200.png";

  const handleLogout = () => {
    // Clear redux data
    dispatch(setCredentials({ user: null, currentPlan: null }));
      ...
      >
        ...
        >
          {({ close }) => (
            <>
              <div className="dark:bg-dark-800 flex items-center gap-4 rounded-t-lg bg-gray-100 px-4 py-5">
                <Avatar size={14} src={logo} alt={profileName} />
                <div>
                  <Link
                    className="hover:text-primary-600 focus:text-primary-600 dark:text-dark-100 dark:hover:text-primary-400 dark:focus:text-primary-400 text-base font-medium text-gray-700"
                    to="/settings/general"
