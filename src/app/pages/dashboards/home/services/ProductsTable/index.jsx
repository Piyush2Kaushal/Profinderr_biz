...
import { getServiceList } from "utils/API/Authapi";
...
export function ServicesTable() {
  ...
  const GetAPI = async () => {
    try {
      ...
      setDataList(Array.isArray(services) ? services : []);
    } catch (error) {
  ...
  const table = useReactTable({
    ...
    meta: {
      deleteRow: (row) => {
        ...
        setDataList((old) => old.filter((oldRow) => oldRow._id !== id));
      },
      deleteRows: (rows) => {
        skipAutoResetPageIndex();
        const ids = rows.map((r) => r.original._id);
        setDataList((old) => old.filter((r) => !ids.includes(r._id)));
      },
    },
